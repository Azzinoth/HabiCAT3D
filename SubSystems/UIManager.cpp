...
void UIManager::RenderLegend()
{
	...
	int screenX = ImGui::GetCurrentWindow()->Pos.x + StartPosition.x;
	int screenY = ImGui::GetCurrentWindow()->Pos.y + StartPosition.y;

	ImColor SomeColor = ImColor(255, 255, 255, 255);

	//ImGui::GetWindowDrawList()->AddRectFilled(ImVec2(screenX + (0.0/*ranges[i].rect.left*/ - StartPosition.x), screenY), ImVec2(0/*beginX*/ + (100.0/*ranges[i].rect.right - ranges[i].rect.left*/), screenY + 20/*size.y*/), SomeColor);
	int LegendW = 300;

	for (size_t i = 0; i < LegendW; i++)
	{
		float factor = i / float(LegendW);
		...
		ImGui::GetWindowDrawList()->AddRectFilled(ImVec2(screenX + i, screenY), ImVec2(i + (1.0), screenY + 20), SomeColor);
	}
