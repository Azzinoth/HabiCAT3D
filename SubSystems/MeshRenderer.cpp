...
glm::vec3 MeshRenderer::GetTurboColorMapValue(float NormalizedValue)
{
	...
	const int Index = int(255 * NormalizedValue);

	return glm::vec3(static_cast<float>(turbo_srgb_floats[Index][0] * 255),
					 static_cast<float>(turbo_srgb_floats[Index][1] * 255),
					 static_cast<float>(turbo_srgb_floats[Index][2] * 255));
}